...
\textbf{LR contribution and comparison scope.} Lei Peng contributed data inspection, preprocessing and the initial LR baseline; earlier result tables are retained. LR* below identifies a separately logged integration run under the shared comparison protocol. Its 12-setting search was specified before execution; models and thresholds were frozen before test evaluation. This \emph{post-hoc harmonization followed earlier test exposure}: it neither replaces the teammate's original execution nor constitutes a fresh independent holdout. Historical test exposure is a team-wide limitation.
\newpage
