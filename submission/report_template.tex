...
\textbf{LR contribution and comparison scope.} Lei Peng contributed data inspection, preprocessing and the initial LR baseline. His original result table reports accuracy {{LR_ORIG_ACC}}, ROC-AUC {{LR_ORIG_AUC}} and recall {{LR_ORIG_REC}} at threshold 0.5 \cite{repo}. The common comparison tables and ranking figure use the separately recorded LR integration run. Its 12-setting search was specified before execution; models and thresholds were frozen before test evaluation. This \emph{post-hoc harmonization followed earlier test exposure}, so it is not a fresh independent holdout. Historical test exposure is a team-wide limitation.
\newpage
