...
\textbf{LR contribution and comparison scope.} Lei Peng contributed data inspection, preprocessing and the initial LR baseline. His original result table reports accuracy {{LR_ORIG_ACC}}, ROC-AUC {{LR_ORIG_AUC}} and recall {{LR_ORIG_REC}} at threshold 0.5 \cite{repo}. LR* identifies the separate integration run used in the common comparison tables and ranking figure. Its 12-setting search was specified before execution; models and thresholds were frozen before test evaluation. This \emph{post-hoc harmonization followed earlier test exposure}, so it is not a fresh independent holdout. Historical test exposure is a team-wide limitation.
\newpage
...
Limitations include a single historical population, random rather than temporal/external evaluation, repeated feature vectors, prior test exposure, unequal search budgets and unvalidated calibration. Subgroup errors and fairness have not been assessed; demographic inputs warrant scrutiny. Hypothetical costs omit exposure, realized losses and intervention benefits. Fixed-prediction intervals exclude model-selection uncertainty. Predictive associations do not establish causality, regulatory compliance or production readiness.
