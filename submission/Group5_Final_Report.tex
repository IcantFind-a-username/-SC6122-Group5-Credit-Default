...
\textbf{LR contribution and comparison scope.} Lei Peng contributed data inspection, preprocessing and the initial LR baseline. His original result table reports accuracy 0.8088, ROC-AUC 0.7101 and recall 0.2442 at threshold 0.5 \cite{repo}. LR* identifies the separate integration run used in the common comparison tables and ranking figure. Its 12-setting search was specified before execution; models and thresholds were frozen before test evaluation. This \emph{post-hoc harmonization followed earlier test exposure}, so it is not a fresh independent holdout. Historical test exposure is a team-wide limitation.
\newpage
