...
NTU SC6122 Emerging Topics in FinTech --- Group 5\hfill 9 September 2026\par
...
\bottomrule\end{tabular}\end{center}
\smallcap{Table 2. Historical test, $n=6,000$, default positive. All rows use threshold 0.5. P/R/F1/Acc are proportions. B = baseline; T = CV-selected; LR* = harmonized integration run (Section 2). Full precision and all confusion counts are in \texttt{model\_comparison.csv}.}
...
\bottomrule\end{tabular}\end{center}
\smallcap{Table 4. Same-model test comparison at 0.5 versus its validation-selected $r=5$ policy. Alert rate is the share flagged for review; costs are total hypothetical units across 6,000 rows. DT has no historical validation-selected cost policy.}
...
\smallcap{Each member has an equal 25\% contribution share. Shared responsibilities cover the data split, preprocessing and metrics, integration of the four sections, result checking and presentation preparation.}
