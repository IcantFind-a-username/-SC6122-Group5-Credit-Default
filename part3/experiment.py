...
def run(output_dir=None, candidates=24, threads=2, bootstrap_repeats=1000):
    ...
    best_id = best_candidate(pd.DataFrame(rows))
    ...
    val_metrics += [{"Model": f"Tuned RF / cost ratio {r}", **metrics(y_val, val_tuned, thresholds[str(r)])} for r in COST_RATIOS]
    ...
    encoded = tuned.named_steps["preprocess"].get_feature_names_out()
    gini = pd.DataFrame({"Feature": encoded,
                         "Gini_importance": tuned.named_steps["model"].feature_importances_})
    gini.sort_values("Gini_importance", ascending=False).to_csv(output / "rf_gini_importance.csv", index=False)
    ...
    protocol = {"Stage": "Frozen before test loading", "Estimator": "RandomForestClassifier",
    ...
    comparison = [{"Model": "Baseline RF", **metrics(y_test, base_p)},
                  {"Model": "Tuned RF", **metrics(y_test, tuned_p)}]
    comparison += [{"Model": f"Tuned RF / cost ratio {r}", **metrics(y_test, tuned_p, thresholds[str(r)])} for r in COST_RATIOS]
