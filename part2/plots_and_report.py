...
def run(output_dir=None):
    ...
    for ax, key, label in zip(axes, ['AP','ROC_AUC'], ['Average Precision (selection metric)', 'ROC-AUC']):
        ...
        ax.errorbar(fixed.Max_depth, fixed['Mean_CV_'+key], yerr=fixed['SD_CV_'+key],fmt='o-',capsize=3,label='CV holdout folds ± SD',color='#db7d22')
    ...
    fig.tight_layout();fig.savefig(figs / '01_overfitting.png',dpi=180);plt.close(fig)

    model = joblib.load(out / 'tuned_pipeline.joblib')
    tree = model.named_steps['model']
    names = model.named_steps['preprocess'].get_feature_names_out()
    fig, ax = plt.subplots(figsize=(19,9))
    plot_tree(tree,feature_names=names,max_depth=2,filled=True,rounded=True,
              impurity=False,fontsize=10,ax=ax)
    depth_label = 'Unlimited' if protocol['Selected_parameters']['max_depth'] is None else str(protocol['Selected_parameters']['max_depth'])
    ax.set_title(f"Selected decision tree: top levels | max_depth={depth_label}, "
                 f"min_samples_leaf={protocol['Selected_parameters']['min_samples_leaf']}",fontsize=17)
    fig.text(.5,.01,'value = [non-default, default] in the 18,000 fitting rows; (...) indicates omitted lower branches.',ha='center',fontsize=11)
    fig.tight_layout(rect=(0,.03,1,1));fig.savefig(figs / '02_tree_top_levels.png',dpi=160);plt.close(fig)
    ...
    for ax, (_, row) in zip(axes,final.iterrows()):
        ...
        for (i,j),value in np.ndenumerate(cm):
            ax.text(j,i,str(value),ha='center',va='center',fontsize=17,color='white' if value>3000 else '#18212c')
        ax.set(xticks=[0,1],yticks=[0,1],xticklabels=['Normal','Default'],yticklabels=['Normal','Default'],
               xlabel='Predicted label',ylabel='Actual label',title=row.Model)
    fig.suptitle('Final test set: 6,000 clients | score ≥ 0.5 predicts default')
    fig.tight_layout();fig.savefig(figs / '03_test_confusion.png',dpi=180);plt.close(fig)

    b,t=final.iloc[0],final.iloc[1]
    columns=['Model','AP','ROC-AUC','Accuracy','Precision','Recall','F1']
    final_table=table(final,columns)
    fit=pd.read_csv(out / 'fit_metrics.csv'); validation=pd.read_csv(out / 'validation_metrics.csv')
    full=pd.concat([fit.assign(Partition='Fit (18,000)'),validation.assign(Partition='Validation (6,000)'),final.assign(Partition='Test (6,000)')])
